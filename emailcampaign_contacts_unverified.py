...
params = urllib.parse.quote_plus(r'Driver={ODBC Driver 17 for SQL Server};Server=tcp:paraforge-dev.database.windows.net,1433;Database=business;Uid=PRD_SVC_ACCT@paraforge.net;Pwd=4pps3rv1c34cct!;Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;Authentication=ActiveDirectoryPassword')
...
def send_email(sender, email_recipient, email_subject, string=None, html=None, attachment_location = ['']):
    ...
    try:
        ...
        text = msg.as_string()
        server.sendmail(email_sender, email_recipient + ['6654912@bcc.hubspot.com'], text)
        print('email sent')
        server.quit()
    except:
...
def get_accrediated_contacts():
    df2 = pd.read_sql(f"""--get user and email format
